...
public:
    ...
    static Matrix<T> bandSolve(Matrix<T>, Matrix<T>, int);
...
template<class T>
Matrix<T> Matrix<T>::solve(Matrix<T> A, Matrix<T> b)
{
    // Gaussian elimination
    for (int i = 0; i < A.rows_; ++i) {
        if (A.p[i][i] == 0) {
            // pivot 0 - throw error
            std::cerr << ("Error: the coefficient matrix has 0 as a pivot. Please fix the input and try again.");
        }
...
template<class T>
Matrix<T> Matrix<T>::bandSolve(Matrix<T> A, Matrix<T> b, int k)
{
